...
class TestFileTool:
    """通过 registry 调用 file 工具（read/write/search）。"""

    def test_file_read_nonexistent(self):
        result = global_registry.dispatch("file", {"action": "read", "path": "/tmp/nonexistent_file_xyz_123"})
        assert "错误" in result

    def test_file_read_sensitive_env(self):
        result = global_registry.dispatch("file", {"action": "read", "path": ".env"})
        assert "拒绝访问" in result or "拒绝" in result

    def test_file_read_sensitive_dot_chips(self):
        result = global_registry.dispatch("file", {"action": "read", "path": ".chips/sessions.db"})
        assert "拒绝" in result

    def test_file_read_empty_path(self):
        result = global_registry.dispatch("file", {"action": "read", "path": ""})
        assert "错误" in result

    def test_file_read_and_write_roundtrip(self, tmp_path):
        p = tmp_path / "test.txt"
        result = global_registry.dispatch("file", {"action": "write", "path": str(p), "content": "hello chips"})
        assert "已写入" in result

        result = global_registry.dispatch("file", {"action": "read", "path": str(p)})
        assert result == "hello chips"

    def test_file_write_append(self, tmp_path):
        p = tmp_path / "append.txt"
        global_registry.dispatch("file", {"action": "write", "path": str(p), "content": "line1\n"})
        global_registry.dispatch("file", {"action": "write", "path": str(p), "content": "line2\n", "mode": "append"})
        result = global_registry.dispatch("file", {"action": "read", "path": str(p)})
        assert result == "line1\nline2\n"

    def test_file_write_sensitive_path(self):
        result = global_registry.dispatch("file", {"action": "write", "path": "/etc/evil.conf", "content": "bad"})
        assert "拒绝" in result

    def test_file_write_sensitive_git(self):
        result = global_registry.dispatch("file", {"action": "write", "path": ".git/HEAD", "content": "hack"})
        assert "拒绝" in result
    ...
    def test_symlink_to_env_is_blocked(self, tmp_path):
        ...
        result = global_registry.dispatch("file", {"action": "read", "path": str(link)})
        assert "拒绝" in result

    def test_resolve_dot_dot(self):
        """Path.resolve() 正确处理 .. 穿越。"""
        result = global_registry.dispatch("file", {"action": "read", "path": "../" * 10 + "tmp"})
        # 穿越到 /tmp → 解析成功，但 /tmp 是目录不是文件
        assert "不是普通文件" in result or "不存在" in result

    def test_non_dot_env_allowed(self):
        """env.txt 不是 .env，不应被误杀。"""
        result = global_registry.dispatch("file", {"action": "read", "path": "env.txt"})
    ...
    def test_etc_prefix_no_false_positive(self, tmp_path):
        ...
        p.write_text("safe")
        result = global_registry.dispatch("file", {"action": "write", "path": str(p), "content": "test"})
        # 不应因为路径包含 "etc" 就被拦截
        assert "拒绝" not in result
...
class TestFileReadLineRange:
    ...
    def test_read_range(self, tmp_path):
        ...
        p.write_text("a\nb\nc\nd\ne\n")
        result = global_registry.dispatch("file", {"action": "read", "path": str(p), "start_line": 2, "end_line": 4})
    ...
    def test_read_single_line(self, tmp_path):
        ...
        p.write_text("alpha\nbeta\ngamma\n")
        result = global_registry.dispatch("file", {"action": "read", "path": str(p), "start_line": 2, "end_line": 2})
    ...
    def test_read_to_end(self, tmp_path):
        ...
        p = tmp_path / "tail.txt"
        p.write_text("1\n2\n3\n4\n5\n")
        result = global_registry.dispatch("file", {"action": "read", "path": str(p), "start_line": 4})
    ...
    def test_line_number_padding(self, tmp_path):
        ...
        result = global_registry.dispatch("file", {"action": "read", "path": str(p), "start_line": 90, "end_line": 100})
        lines = result.split("\n")
        assert len(lines) == 11
...
class TestFilePatch:
    ...
    def test_patch_replace_first_occurrence(self, tmp_path):
        ...
        p.write_text("1\nTARGET\n2\nTARGET\n3\n")
        result = global_registry.dispatch("file", {"action": "write",
...
class TestFileSearch:
    ...
    def test_search_text_in_file(self, tmp_path):
        ...
        result = global_registry.dispatch("file", {"action": "search",
            "path": str(tmp_path),
            "pattern": "hello",
        })
    ...
    def test_search_empty_pattern(self):
        result = global_registry.dispatch("file", {"action": "search", "pattern": ""})
        assert "错误" in result
...
class TestWebFetch:
    ...
    def test_invalid_scheme(self):
        result = global_registry.dispatch("web", {"action": "fetch", "url": "ftp://example.com"})
