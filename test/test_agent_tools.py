"""Test orchestrate — 子 Agent 委派（mode=single）
...
class TestOrchestrateSingle:
    """验证 orchestrate(mode=single) 的基本行为。"""
    ...
    def test_wire_and_delegate(self, mock_gateway):
        """基本链路：wire → orchestrate(mode=single) → 返回子 Agent 结果。"""
        ...
        result = orch({"mode": "single", "task": "帮我查一下今天的天气"})
    ...
    def test_task_empty(self, mock_gateway):
        ...
        result = json.loads(orch({"mode": "single", "task": ""}))
    ...
    def test_with_tools(self, mock_gateway):
        ...
        result = orch({"mode": "single", "task": "搜索信息", "tools": ["web"]})
        assert "子任务已完成" in result
    ...
    def test_with_model(self, mock_gateway):
        ...
        result = orch({"mode": "single", "task": "测试", "model": "gpt-4o-mini"})
        assert "子任务已完成" in result
    ...
    def test_not_wired(self):
        """未 wire 时返回 error。"""
        from tool.builtins.orchestrate_tool import _handle as orch
        result = orch({"mode": "single", "task": "测试"})
...
class TestAgentRegistry:
    ...
    def test_registry_agent_resolved(self, mock_gateway, mock_registry):
        ...
        result = orch({"mode": "single", "agent": "researcher", "task": "搜索"})
        assert "子任务已完成" in result
    ...
    def test_registry_unknown_agent(self, mock_gateway, mock_registry):
        ...
        result = json.loads(orch({"mode": "single", "agent": "hacker", "task": "测试"}))
        assert "error" in result
    ...
    def test_registry_not_wired(self, mock_gateway):
        ...
        result = json.loads(orch({"mode": "single", "agent": "researcher", "task": "测试"}))
        assert "error" in result
