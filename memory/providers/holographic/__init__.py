"""HolographicMemoryProvider — SQLite fact store with HRR retrieval.

Provides fact_store and fact_feedback tools for structured durable memory
with entity resolution, trust scoring, and Holographic Reduced Representation
algebraic retrieval.

Adapter of the original Hermes holographic plugin for chips-agent.
"""
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self._store: HolographicStore | None = None
    ...
    def initialize(self, session_id: str = "", **kwargs) -> None:
        ...
        if not db_path:
            base = os.getenv("CHIPS_MEMORY_DIR", ".memory")
            db_path = os.path.join(base, "holographic.db")
        ...
        self._store = HolographicStore(db_path=db_path, default_trust=default_trust, hrr_dim=hrr_dim)
        ...
        try:
            ...
            if action == "add":
                ...
                results = retriever.reason(
                    entities, category=args.get("category"),
                    limit=int(args.get("limit", 10)),
                )
                return json.dumps({"results": results, "count": len(results)})
